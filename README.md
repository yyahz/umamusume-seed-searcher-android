...
当前版本为 `v0.1.50`，仍处于测试阶段。

本版更新：退出后重新进入可恢复已下载新版的安装入口；更新前优先复用通过包名、版本和签名校验的本地安装包。完整安装包保存在应用私有、免备份目录，旧版本缓存中的完整安装包也可复用；损坏或已安装版本的包不作为更新使用，未完成下载不复用。
...
- 使用“角色 / 因子 / 结果 / 设置”四页式手机界面；顶部品牌信息仅在角色搜索入口显示，设置页提供版本更新与数据服务声明。
...
```text
app/build/outputs/apk/debug/uma-seed-searcher-android-v0.1.50-debug.apk
```
